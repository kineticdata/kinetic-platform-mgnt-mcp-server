...
export function createKineticMcpServer(context) {
    ...
    const invoke = (sessionId, op, input) => invokeDefaultOperation({ getClient: (sid, api) => getOrCreateClient(context, sid, api) }, sessionId, op, input);
    if (mode === "slim") {
...
function describeCoverage(mode, context) {
